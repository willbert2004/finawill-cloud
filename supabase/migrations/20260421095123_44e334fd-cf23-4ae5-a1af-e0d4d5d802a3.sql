...
BEGIN
  ...
  -- Insert one pending rating per matching supervisor (case-insensitive overlap)
  ...
  -- Fallback: if no overlap matches, send to all supervisors in the same department (case-insensitive)
  IF inserted_count = 0 AND proj.department IS NOT NULL THEN
    INSERT INTO public.project_ratings (project_id, supervisor_id, status)
    SELECT _project_id, s.user_id, 'pending'
    FROM public.supervisors s
    WHERE lower(trim(s.department)) = lower(trim(proj.department))
    ON CONFLICT (project_id, supervisor_id) DO NOTHING;
    GET DIAGNOSTICS inserted_count = ROW_COUNT;
  END IF;
